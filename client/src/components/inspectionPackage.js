...
function InspectionPackage(props) {
    ...
    return (
        <div className="card" style={{ width: '22rem', textAlign: 'center', border:'none'  }}>
            <div className="card-body bg-dark" style={{ padding: '17px 0', color: 'white', lineHeight: '20px' }}>
                <h2 className="card-title"><b>Inspection Package</b></h2>
                <p><span style={{ fontSize: '2.1rem' }}>4000</span> Starts From</p>
            </div>
            <ul className="list-group list-group-flush">
                {service.map((item) => {
                    return (
                        <li style={{textAlign:'left',fontSize:'15px',backgroundColor:'#EEEEEE' }} key={item.packageID} className="list-group-item">🛠️{item.inspectionPackage}</li>
                    );
                })}
            </ul>
